...
\begin{itemize}[leftmargin=6.5em]
	...
	针对高级用户，我们提供基于Lua语言的代码编辑器，如\prettyref{fig:代码编辑器}。使用代码编辑器可以完成各种复杂的逻辑和自定义的操作流程，可定制化程度高，需要具备一定的编程基础和逻辑基础。
...
\end{itemize}
	...
	\begin{figure}[htb]
		...
		\caption{时间轴编辑器\protect\footnotemark}
		\label{fig:时间轴编辑器}
...
参考坐标系选择\mnu{相对于底座}，表示以机器人底座平面的圆心作为世界坐标系原点。
...
速度与加速时间的调整，用户可以在场景编辑器的工具栏上全局操作（见\prettyref{sec:速度和加速时间}），也可以对每个位置块单独操作。
...
点击场景编辑器功能区的\btn{批量修改…}按钮，或者用鼠标右键点击动作块选择\mnu{批量修改…}，可对动作块以下内容进行批量编辑。
...
\ttt 2个或以上的位置动作块，点击编辑区的\mnu{批量微调位置…}，或者将光标放在选中的位置动作块上使当前位置动作块处于焦点状态，点击鼠标右键，选择\mnu{批量微调位置…}，进入\mnu{调整位置}页面（详见\prettyref{sec:微调}）。
